...
{
    public class HentaiSpearBigDeathrayLegacy : BaseDeathrayLegacy
    {
        public HentaiSpearBigDeathrayLegacy() : base(60, "PhantasmalDeathrayML", hitboxModifier: 1.5f) { }
        ...
        public override void AI()
        {
            ...
            if (!Main.dedServ && Main.LocalPlayer.active)
                Main.LocalPlayer.GetModPlayer<HentaiSpearBigDeathrayPlayerLegacy>().Screenshake = 2;
            ...
            int byUUID = FargoSoulsUtilLegacy.GetByUUIDReal(Projectile.owner, (int)Projectile.ai[1], ModContent.ProjectileType<HentaiSpearWandLegacy>());
            if (byUUID != -1)
            {
            ...
            {
                SoundStyle Zombie_104 = new SoundStyle("FargoLegacy/Sounds/Zombie_104");
                if (!Main.dedServ)
                    Terraria.Audio.SoundEngine.PlaySound(Zombie_104, Projectile.Center);
            }
            float num801 = 10f;

            if (Projectile.localAI[0] == maxTime / 2)
            {
                if (Projectile.owner == Main.myPlayer && !(player.controlUseTile && player.altFunctionUse == 2 && player.HeldItem.type == ModContent.ItemType<Content.Items.Weapon.HentaiSpearLegacy>()))
            ...
            if (++Projectile.ai[0] > 60)
            {
                ...
                if (Projectile.owner == Main.myPlayer)
                {
                    ...
                    for (int i = 0; i < ringMax; i++)
                    {
                        ...
                        Projectile.NewProjectile(Projectile.GetSource_FromThis(), player.Center, vel, ModContent.ProjectileType<HentaiSphereRingLegacy>(),
    ...
    public partial class HentaiSpearBigDeathrayPlayerLegacy : ModPlayer
    {
