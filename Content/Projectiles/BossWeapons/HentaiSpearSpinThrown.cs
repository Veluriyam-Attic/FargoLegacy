...
using FargoLegacy.Content.Items.Weapon;
...
{
    public class HentaiSpearSpinThrownLegacy : ModProjectile
    {
            ...
            if (++Projectile.localAI[0] > 10)
            {
                ...
                if (Projectile.owner == Main.myPlayer)
                {
                    ...
                    int p = Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.position + Main.rand.NextVector2Square(0f, Projectile.width),
                        speed, ModContent.ProjectileType<PhantasmalEyeHomingLegacy>(), Projectile.damage, Projectile.knockBack / 2, Projectile.owner, -1, ai1);
            ...
            for (int i = 0; i < Main.maxProjectiles; i++)
            {
                    ...
                    && FargoSoulsUtil.CanDeleteProjectile(Main.projectile[i]))
                {
                    if (Projectile.owner == Main.myPlayer)
                    {
                        ...
                        Projectile.NewProjectile(Projectile.GetSource_FromThis(), Main.projectile[i].Center, Vector2.Zero, ModContent.ProjectileType<IronParry>(), 0, 0f, Main.myPlayer);
                    }
        ...
        public override void Kill(int timeLeft) //self reuse so you dont need to hold up always while autofiring
        {
            if (Projectile.owner == Main.myPlayer && Main.player[Projectile.owner].controlUseTile && Main.player[Projectile.owner].altFunctionUse == 2
                ...
                && Main.player[Projectile.owner].HeldItem.type == ModContent.ItemType<Content.Items.Weapon.HentaiSpearLegacy>()
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            {
                Player player = Main.player[Projectile.owner];
                Texture2D glow = ModContent.Request<Texture2D>("FargoLegacy/Content/Projectiles/BossWeapons/HentaiSpearSpinGlowLegacy", ReLogic.Content.AssetRequestMode.ImmediateLoad).Value;
