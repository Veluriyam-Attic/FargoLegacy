using FargoLegacy.Content.Buffs.Souls;
...
namespace FargoLegacy.Content.Projectiles.Minions
{
    public class HallowSwordLegacy : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (player.whoAmI == Main.myPlayer && (player.dead || !player.HasEffect<AncientHallowMinionLegacy>()))
            {
                Projectile.Kill();
                return;
            }
        ...
        private void Slash(Player player)
        {
            ...
            Reflected = false;
            if (!player.HasBuff<HallowCooldownBuffLegacy>())
            {
                Reflect(Projectile);
            }
        ...
        private void Reflect(Projectile sword)
        {
            ...
            int damageCap = player.FargoSouls().ForceEffect<AncientHallowEnchantLegacy>() ? 200 : 150;
            ...
            foreach (Projectile p in Main.projectile.Where(p => p.active && p.hostile && p.damage > 0 && FargoSoulsUtil.CanDeleteProjectile(p) && p.damage <= damageCap && sword.Colliding(sword.Hitbox, p.Hitbox)))
            {
                ...
                p.hostile = false;
                p.friendly = true;
                player.AddBuff(ModContent.BuffType<HallowCooldownBuffLegacy>(), 60 * 15);
                p.owner = sword.owner;
        ...
        private static void DrawProj_EmpressBlade(Projectile proj, float hueOverride)
        {
            ...
            HallowSwordLegacy sword = proj.ModProjectile != null && proj.ModProjectile is HallowSwordLegacy ? proj.As<HallowSwordLegacy>() : null;
